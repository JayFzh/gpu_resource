...
# default policy
def default(model: dl_model, default_thd_option = 2048):
...
def dp_fusion(model: dl_model):
    ...
    while last_hop[0] != -1:
        ...
        comm_start_tp.insert(0, trace_state[0])
    #'''
    #layer_fusion = [9, 23, 0, 0,0, 0]
    #thread_allocation = [1024, 4096, 256, 256, 256, 256]
...
MAX_FUSION_NUM = 12 # max number of groups for fusion num
if __name__ == "__main__":
    test_model = [["VGG16", 32], ["VGG16", 4], ["Xception", 8], ['Bert', 2], ["ResNet152", 8]][-1]
    algorithm = ["dp", "dp-fusion", "bayes", "default", "merge-wfbp", "default-random-fusion", "EASGD"][1]
    fusion_required = ["dp", "bayes", "default"]
    ...
    fusion_group =  None#[1, 4, 27] #[9, 23]#[1, 9, 4, 2, 4, 2, 2, 2, 2, 2, 2] #[1, 6, 1, 1, 1, 2, 2, 1, 1, 2, 1, 1, 1, 1, 1, 1, 2, 2, 2, 2]#[1, 3, 4, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2]##[4, 4, 4, 8, 8, 4]#[4 for i in range(8)] # [7, 20, 5]  [512, 512, 4096]      [2 for i in range(len(pids) // 2)]
    ...
    if algorithm == "EASGD":
        ...
        thread_allocation, opt_time = default(model)# then using default resource policy
    elif algorithm == "dp":
